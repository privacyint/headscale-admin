<script lang="ts">
	...
	const layout = $derived(layoutGraph(filtered, { rowSpacing: 56 }));
	const selectedNeighbours = $derived(
		selectedId ? neighboursOf(filtered, selectedId) : new Set<string>(),
	);
	...
	<div class="grid grid-cols-1 lg:grid-cols-4 gap-4" data-testid="visualise-root">
		<aside class="card p-4 lg:col-span-1 space-y-4" data-testid="visualise-filters">
						...
						<li class="flex items-center justify-between">
							<label class="flex items-center gap-2 text-sm">
								...
								<span
									...
									style="background: {kindColour[k.key]}"
								></span>
								{k.label}
							</label>
						</li>
			...
			<div class="text-xs opacity-70" data-testid="visualise-counts">
				Entities: {filtered.nodes.length} · Relations: {filtered.edges.length}
			</div>
		</aside>
					...
					>
							...
							{#each filtered.edges as e (e.id)}
								...
								{#if from && to}
									<line
										...
										stroke-width={e.kind === 'acl-accept' ? 2 : 1}
										stroke-dasharray={e.kind === 'tag-node' ? '4 4' : undefined}
						...
						<g class="nodes">
							{#each layout.nodes as n (n.id)}
								...
								>
									<circle
										r={selectedId === n.id ? 14 : 10}
										fill={kindColour[n.kind]}
										stroke="white"
										stroke-width="2"
									/>
									<text x="16" y="4" font-size="12" fill="currentColor" class="select-none">
										{n.label}
									</text>
								</g>
							{/each}
				...
				{#if selected}
					<div class="flex items-center justify-between mb-2">
						<div class="flex items-center gap-2">
							<span
								class="inline-block w-3 h-3 rounded-full"
								style="background: {kindColour[selected.kind]}"
							></span>
					...
					<div class="text-sm">
						...
						{:else}
							<ul class="flex flex-wrap gap-2">
								{#each filtered.nodes.filter((n) => selectedNeighbours.has(n.id)) as n}
									<li>
										<button
											type="button"
										...
										>
											<span
												...
												style="background: {kindColour[n.kind]}"
											></span>
											{n.label}
										</button>
									</li>
